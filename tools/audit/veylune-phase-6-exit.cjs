...
const contractPath = process.argv[3] || path.resolve(process.cwd(), 'config/veylune-phase-6-exit-contract.json');
...
const outputRoot = process.env.VEYLUNE_PHASE6_EXIT_OUTPUT || path.resolve(projectRoot, contract.evidenceDirectory);
...
async function inspectPage(page) {
    ...
    return page.evaluate(() => {
        ...
        return {
            ...
            primaryActionLabels: [...document.querySelectorAll('[data-pdp-add-selection], [data-pdp-add-selection-mobile]')]
                .map((element) => (element.textContent || '').trim()),
            semanticPdp: Boolean(document.querySelector('[data-veylune-pdp-semantic]')),
            semanticFacts: document.querySelectorAll('.veylune-pdp-semantic__facts > div').length,
            productFamily: document.querySelector('[data-veylune-pdp-preview]')?.dataset.pdpFamily
                || document.querySelector('[data-veylune-pdp-semantic]')?.dataset.pdpFamily || '',
            commerceState: document.querySelector('[data-veylune-pdp-preview]')?.dataset.commerceState
                || document.querySelector('[data-veylune-pdp-semantic]')?.dataset.commerceState || '',
            journeyContracts: document.querySelectorAll('[data-commerce-journey]').length,
            noVerifiedReviewsCopy: document.body.textContent.includes('No verified reviews yet'),
            noBindingOrderCopy: /does not reserve stock, collect payment or create an order|no (public )?order is created|non-binding private preview/i.test(document.body.textContent),
        };
    });
}
...
async function privateAccountStateScenario(browser, viewportDefinition, seedProduct) {
    const context = await browser.newContext({
        viewport: { width: viewportDefinition.width, height: viewportDefinition.height },
        ignoreHTTPSErrors: true,
        reducedMotion: 'reduce',
    ...
    await context.addInitScript(({ product, mediaUrl }) => {
        const lineId = `${product.recordId}::account-material`;
        window.localStorage.setItem('veylune-private-selection-v2', JSON.stringify({
            version: 2,
            items: [{
                lineId,
        ...
        window.localStorage.removeItem('veylune-private-projects-v1');
    }, { product: seedProduct, mediaUrl: seedProduct.media?.mediaUrl || '' });

    const page = await context.newPage();
    const errors = emptyErrors();
    attachErrors(page, errors);
    const response = await page.goto(urlFor('/__veylune-preview/account', true), { waitUntil: 'domcontentloaded', timeout: 45000 });
    const saveButton = page.locator('[data-account-project-save]');
    const saveEnabled = await saveButton.isEnabled();
    await saveButton.click();
    const savedProjects = await page.locator('[data-account-project-list] > .veylune-account-project-card').count();
    const saveNotice = (await page.locator('[data-account-project-notice]').textContent()) || '';

    await page.locator('[data-account-orders-open]').first().click();
    const ordersOpened = await page.locator('[data-account-orders-dialog]').evaluate((dialog) => dialog.open);
    await page.locator('[data-account-orders-close]').click();
    await page.locator('[data-account-readiness-open]').first().click();
    const readinessOpened = await page.locator('[data-account-readiness-dialog]').evaluate((dialog) => dialog.open);
    const readinessItems = await page.locator('[data-account-readiness-list] > *').count();
    await page.locator('[data-account-readiness-close]').click();

    const pass = response?.status() === 200 && saveEnabled && savedProjects === 1
        && /saved/i.test(saveNotice) && ordersOpened && readinessOpened && readinessItems === 4
        && errors.consoleErrors.length === 0 && errors.pageErrors.length === 0 && errors.failedResponses.length === 0;
    await context.close();
    return {
        viewport: viewportDefinition.id,
...
async function publicAccountValidationScenario(browser, viewportDefinition, definition) {
    const context = await browser.newContext({
        viewport: { width: viewportDefinition.width, height: viewportDefinition.height },
        ignoreHTTPSErrors: true,
        reducedMotion: 'reduce',
    ...
    const errors = emptyErrors();
    attachErrors(page, errors);
    const response = await page.goto(urlFor(definition.route), { waitUntil: 'domcontentloaded', timeout: 45000 });
    const validation = await page.evaluate(() => {
        ...
        const forms = [...document.querySelectorAll('main form')].filter(visible);
        const candidates = forms.map((form) => ({
            form,
            fields: [...form.querySelectorAll('input:not([type="hidden"]), select, textarea')].filter(visible),
            required: [...form.querySelectorAll('input[required], select[required], textarea[required]')].filter(visible),
        })).filter((candidate) => candidate.fields.length > 0).sort((left, right) => right.fields.length - left.fields.length);
        const candidate = candidates[0];
        if (!candidate) return { formFound: false, inputFields: 0, namedLabeledFields: 0, requiredFields: 0, initialInvalid: false, submitControl: false, serverValidationPath: false };
        const namedLabeledFields = candidate.fields.filter((field) => {
            const id = field.id || '';
            const labelled = (field.getAttribute('aria-label') || '').trim()
            ...
            return Boolean((field.getAttribute('name') || id) && labelled);
        }).length;
        return {
            formFound: true,
            inputFields: candidate.fields.length,
            namedLabeledFields,
            requiredFields: candidate.required.length,
            initialInvalid: candidate.required.length > 0 ? !candidate.form.checkValidity() : null,
            submitControl: Boolean(candidate.form.querySelector('button[type="submit"], input[type="submit"]')),
            serverValidationPath: (candidate.form.method || '').toLowerCase() === 'post' && Boolean(candidate.form.action),
        };
    });
    const pass = response?.status() === definition.expectedStatus && validation.formFound
        && validation.inputFields > 0 && validation.inputFields === validation.namedLabeledFields
        && validation.submitControl && validation.serverValidationPath
        && (validation.requiredFields === 0 || validation.initialInvalid)
        && errors.consoleErrors.length === 0 && errors.pageErrors.length === 0 && errors.failedResponses.length === 0;
    await context.close();
    return {
        viewport: viewportDefinition.id,
...
async function run() {
    ...
    const report = {
        ...
        privateAccountStateScenarios: [],
        publicAccountValidationScenarios: [],
        publicProductBoundaries: [],
        unauthorizedPrivateRoutes: [],
        externalActivationGates: contract.externalActivationGates,
    };
    ...
    for (const viewportDefinition of contract.viewports) {
        ...
        report.privateAccountStateScenarios.push(await privateAccountStateScenario(browser, viewportDefinition, products[0]));
        for (const definition of contract.publicSurfaceRoutes.filter((item) => ['account-login', 'account-register', 'account-recover'].includes(item.id))) {
            report.publicAccountValidationScenarios.push(await publicAccountValidationScenario(browser, viewportDefinition, definition));
        }
    }
    ...
    const privatePdpSurfaces = report.surfaces.filter((item) => item.scope === 'private-pdp');
    const privateJourneySurfaces = report.surfaces.filter((item) => item.scope === 'private-journey');
    const semanticPdpSurfaces = privatePdpSurfaces.filter((item) => item.computed.semanticPdp && item.computed.semanticFacts > 0).length;
    const truthfulPrimaryActionSurfaces = privatePdpSurfaces.filter((item) => item.computed.commerceState === 'non-binding-preview'
        && item.computed.primaryActionLabels.length > 0
        && item.computed.primaryActionLabels.every((label) => label === 'Add to preview cart')
        && item.computed.noBindingOrderCopy).length;
    const journeyContractSurfaces = privateJourneySurfaces.filter((item) => item.computed.journeyContracts === 1).length;
    report.summary = {
        products: products.length,
        viewports: contract.viewports.length,
        privatePdpSurfaces: privatePdpSurfaces.length,
        privateJourneySurfaces: privateJourneySurfaces.length,
        ...
        semanticPdpSurfaces,
        truthfulPrimaryActionSurfaces,
        journeyContractSurfaces,
        productFamilies: new Set(privatePdpSurfaces.map((item) => item.computed.productFamily).filter(Boolean)).size,
        productTypeKeys: new Set(report.productInventory.map((item) => item.productType)).size,
        ...
        privateAccountStateScenarios: report.privateAccountStateScenarios.length,
        failedPrivateAccountStateScenarios: report.privateAccountStateScenarios.filter((item) => !item.pass).length,
        publicAccountValidationScenarios: report.publicAccountValidationScenarios.length,
        failedPublicAccountValidationScenarios: report.publicAccountValidationScenarios.filter((item) => !item.pass).length,
        ...
        externalActivationGates: report.externalActivationGates.length,
        screenshots: fs.readdirSync(outputRoot).filter((file) => file.endsWith('.png')).length,
    };
    const implementationChecks = [
        ['statusExpectationFailures', report.summary.statusExpectationFailures === 0],
        ['runtimeIssueSurfaces', report.summary.runtimeIssueSurfaces === 0],
        ['overflowSurfaces', report.summary.overflowSurfaces === 0],
        ['brokenImageSurfaces', report.summary.brokenImageSurfaces === 0],
        ['accessibilityIssueSurfaces', report.summary.accessibilityIssueSurfaces === 0],
        ['semanticPdpSurfaces', report.summary.semanticPdpSurfaces === contract.expected.semanticPdpSurfaces],
        ['truthfulPrimaryActionSurfaces', report.summary.truthfulPrimaryActionSurfaces === contract.expected.truthfulPrimaryActionSurfaces],
        ['journeyContractSurfaces', report.summary.journeyContractSurfaces === contract.expected.journeyContractSurfaces],
        ['failedPrivatePdpInteractions', report.summary.failedPrivatePdpInteractions === 0],
        ['failedPrivateJourneyScenarios', report.summary.failedPrivateJourneyScenarios === 0],
        ['failedPrivateAccountStateScenarios', report.summary.failedPrivateAccountStateScenarios === 0],
        ['failedPublicAccountValidationScenarios', report.summary.failedPublicAccountValidationScenarios === 0],
        ['failedPublicProductBoundaryChecks', report.summary.failedPublicProductBoundaryChecks === 0],
        ['failedUnauthorizedPrivateRouteChecks', report.summary.failedUnauthorizedPrivateRouteChecks === 0],
    ];
    report.implementationChecks = implementationChecks.map(([id, pass]) => ({ id, pass }));
    report.summary.unresolvedImplementationGaps = report.implementationChecks.filter((item) => !item.pass).length;

    fs.writeFileSync(path.join(outputRoot, 'phase-6-exit.json'), `${JSON.stringify(report, null, 2)}\n`);
